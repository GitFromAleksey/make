 ...
 * \return None
 *
 * The functions IfxCpu_waitEvent and IfxCpu_emitEvent are used to achieve synchronisation between all cores (i.e individual cores wait till all cores have reached the synchronisation point).
 *
 * \code
 ...
 * IfxCpu_waitEvent(&event, timeoutMilliSec); // timeoutMilliSec is timeout value to wait
 *
 * \endcode
 *
 */
IFX_EXTERN void IfxCpu_waitEvent(IfxCpu_syncEvent *event, uint32 timeoutMilliSec);
