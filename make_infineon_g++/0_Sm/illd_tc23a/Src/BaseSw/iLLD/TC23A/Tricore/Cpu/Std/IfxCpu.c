...
void IfxCpu_waitEvent(IfxCpu_syncEvent *event, uint32 timeoutMilliSec)
{
    ...
    while ((*sync != IFXCPU_CFG_ALLCORE_DONE) &&
...
{
    __imaskldmst(event, IFXCPU_CFG_ALLCORE_DONE, __mfcr(CPU_CORE_ID), 1);
}
