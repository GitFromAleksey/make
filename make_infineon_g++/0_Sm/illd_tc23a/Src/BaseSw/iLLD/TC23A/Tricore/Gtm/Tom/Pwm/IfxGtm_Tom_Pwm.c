...
void IfxGtm_Tom_Pwm_start(IfxGtm_Tom_Pwm_Driver *driver, IfxGtm_Tom_Pwm_Config *config)
{
    /* Enable channel if not enabled already */
    IfxGtm_Tom_Tgc_enableChannel(driver->tgc[0], driver->tomChannel, TRUE, config->synchronousUpdateEnabled == 0 ? TRUE : FALSE);       // Blessen update for app pattern
    IfxGtm_Tom_Tgc_enableChannelOutput(driver->tgc[0], driver->tomChannel, TRUE, config->synchronousUpdateEnabled == 0 ? TRUE : FALSE); // Blessen udpate for app pattern
...
void IfxGtm_Tom_Pwm_stop(IfxGtm_Tom_Pwm_Driver *driver, IfxGtm_Tom_Pwm_Config *config)
{
    /* Disable channels */
    IfxGtm_Tom_Tgc_enableChannel(driver->tgc[0], driver->tomChannel, FALSE, config->synchronousUpdateEnabled == 0 ? TRUE : FALSE);       // Blessen update for app pattern
    IfxGtm_Tom_Tgc_enableChannelOutput(driver->tgc[0], driver->tomChannel, FALSE, config->synchronousUpdateEnabled == 0 ? TRUE : FALSE); // Blessen udpate for app pattern
